...
import wallAccretionSublimationFlags
...
simulate_wall_accretion = config.getboolean("SIMULATION", "simulate_wall_accretion")
simulate_wall_sublimation = config.getboolean("SIMULATION", "simulate_wall_sublimation")
...
print(f"Simulate wall accretion: {simulate_wall_accretion}", flush=True)
print(f"Simulate wall sublimation: {simulate_wall_sublimation}", flush=True)
if simulate_wall_accretion or simulate_wall_sublimation:
    print(f"Maximum number of runs: {maximum_number_of_runs}", flush=True)
else:
    print("Wall interactions not simulated, maximum number of runs set to 0.", flush=True)
    maximum_number_of_runs = 0
...
# --------------------------------------------------
# Synchronize wall accretion and sublimation flags in C++ header file
header_path = os.path.join(home, "src/solver")
os.chdir(header_path)
wallAccretionSublimationFlags.sync(simulate_wall_accretion, simulate_wall_sublimation)
...
while simulation_time < (simulation_end_time * 60) and number_of_runs <= maximum_number_of_runs:
    ...
    monitorSimulation.check_rms(convergence_thresholds, steady_state_simulation_end_time_limit)
    ...
    if not simulate_wall_accretion and not simulate_wall_sublimation:
        print("Wall interactions not simulated, skipping wall evolution step.", flush=True)
    ...
    while np.all(np.abs(dRw_total) < R_wall_threshold) and not wall_closed:
        ...
        if np.any(wall_coordinates_temp[:,1] <= 0):
            ...
            with open(f"./simulation_results/{simulation_time:.2f}_wall_closed/wall_coordinates.dat", 'w') as f:
